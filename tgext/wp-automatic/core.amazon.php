					...
					if (trim ( $keyword ) != '') {
						...
						if (count ( $res ) > 0) {
							...
							*/
							...
							$chart_url = "http://www.amazon.{$camp->camp_amazon_region}/gp/aws/cart/add.html?AssociateTag=$amazonAid&ASIN.1=$asin&Quantity.1=1&SubscriptionId=$subscription";
			...
			function amazon_fetch_links($keyword, $camp) {
				...
				if($start > $maxApiPages && trim($moreUrl) != ''){
					...
					try {
						...
						echo '<br>Scraping of the search page returned '. count($ASINs);
