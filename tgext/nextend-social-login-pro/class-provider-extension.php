...
class NextendSocialLoginPROProviderExtension {
            ...
            if (is_array($disable_roles) && count($disable_roles) > 0) {
                ...
                foreach ($user_info->roles AS $user_role) {
                    if (in_array($user_role, $disable_roles)) {
                        ...
                        \NSL\Notices::addError(__('Social login is not allowed with this role!', 'nextend-facebook-connect'));
                        $isAllowed = false;
                    }
                }
            }
        ...
        <p>
            <label for="user_login"><?php _e('Username') ?><br/>
                <input type="text" name="user_login" id="user_login" class="input"
                       value="<?php echo esc_attr(wp_unslash($userData['user_login'])); ?>" size="20"/></label>
        </p>
        <?php
    }
    ...
    public function isAutoLinkAllowed($isAllowed) {
        if ($this->provider->settings->get('auto_link') == 'disabled') {
            ...
            \NSL\Notices::addError(sprintf(__('This email is already registered, please login in to your account to link with %1$s.', 'nextend-facebook-connect'), $this->provider->getLabel()));
