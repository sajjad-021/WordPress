...
class WC_Braintree_CC_Payment_Gateway extends WC_Braintree_Payment_Gateway {
	...
	public function add_transaction_attributes($args, $order, $gateway_id) {
		if ($gateway_id === $this->id) {
			...
			/**
			 * Unset the WC billing fields postal code from billing address if the postal code from the
			 * dropin form or custom forms is being used.
			 * The payment nonce will have the form's postal code already.
			 */
			if (! $this->should_save_payment_method () && in_array ( 'postal_code', $this->get_config_data ()[ 'challenges' ] )) {
